    ...
    <div className="flex flex-col gap-3 md:flex-row md:items-center md:gap-0">
      {steps.map((step, i) => (
        <div key={step} className="flex flex-col items-center gap-3 md:flex-1 md:flex-row">
          <div className="w-full rounded-sm border border-line bg-navy-secondary px-5 py-4 text-center md:flex-1">
            <span className="text-[14px] font-semibold text-offwhite">{step}</span>
          </div>
          {i < steps.length - 1 && (
